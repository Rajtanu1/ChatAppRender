  ...
  return (
    <form id="logIn-form-container" onSubmit={props.login} autoComplete="off">
      ...
      <div id="google-button">
        <p id="or">Or</p>
        <a href={`https://${window.location.hostname}:3000/auth/google`} id="google-logIn-button"><i className="fa-brands fa-google"></i>&nbsp;Login in with Google</a>
      </div>
    </form>
  )
}
...
class FormContainer extends React.Component {
   ...
   passwordVisibilityToggle(event) {
    ...
    formSubmission(event) {
      ...
      if(confirmPasswordErrDiv.textContent === "") {
       httpRequest.open('POST', `https://${window.location.hostname}:3000/form`, true);
   ...
   loginHandler(event) {
     ...
     httpRequest.open("POST", `https://${window.location.hostname}:3000/login`, true);
     httpRequest.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
     httpRequest.send(JSON.stringify(loginData));
     httpRequest.onload = function(event) {
       if(httpRequest.readyState === 4 && httpRequest.status === 200) {
         window.location.href = `https://${window.location.hostname}:3000/homepage`;
       } else {
